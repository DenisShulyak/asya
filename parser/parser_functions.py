...
def login_button(browser):
    try:
        login_button = WebDriverWait(browser, 1).until(
...
# Дополненная функция для сохранения файла в папку parsed_data
def saveToFile(json_data, filename=None):
    if not filename:
        filename = "data"
    
    # Определяем путь к папке parsed_data
    ...
    file_path = os.path.join(parsed_data_dir, f"{filename}.json")

    # Сохранение файла
    with open(file_path, "w", encoding="utf-8") as file:
        file.write(json_data)
    
    print(f"Файл '{filename}.json' успешно сохранен в папку {parsed_data_dir}")
...
def start_parser(browser, speciality_links, speciality_link_index):
    ...
    for diagnosis_link in diagnosies_links:
        ...
        for diagnosis_block in diagnosis_blocks:
            ...
            diagnosis.add_block(
                Block(name=diagnosis_block_name.text, comment=comment_html, items=items, titles=titles))
